...
contacts<-read.xlsx("data/New York State Office of Information Technology Services/Sampling Season - Documents/LCI.Field.Season.Intensive.xlsx" , 6, header=TRUE)
...
holders<-read.xlsx("data/New York State Office of Information Technology Services/Sampling Season - Documents/LCI.Field.Season.Intensive.xlsx" , 6, header=TRUE)
...
rm(list=c('access','time','float','contacts','holders'))

sampling<-sampling[!is.na(sampling$basin),]
sampling$Time<-gsub("1899-12-30 ","",sampling$Time)
sampling$hours<-gsub("1899-12-30 ","",sampling$hours)
...
sampling$hours<-ifelse(is.na(sampling$hours),sampling$time,sampling$hours)
...
sampling$hours<-hhmmss2dec(sampling$hours)
sampling$time.to.drive.to.next<-hhmmss2dec(sampling$time.to.drive.to.next)

#add return time
sampling$Return<-sampling$Departure+(sampling$hours*60*60)
sampling$startnext<-sampling$Return+(sampling$time.to.drive.to.next*60*60)

#set departure time to previous startnext
sampling<-sampling %>% 
  group_by(basin,Trip,team,Day) %>% 
  mutate(start=lag(startnext,n=1L,default=NA,order_by=Order)) %>% 
  ungroup()
sampling$Departure<-if_else(!is.na(sampling$start),sampling$start,sampling$Departure)

#now calcuate the new return time
sampling$Return<-sampling$Departure+(sampling$hours*60*60)

sampling<-sampling[sampling$basin=="Long Island",]
sampling<-sampling[sampling$Trip=="1",]
...
Departure Date,Time,Location: %s, Central Office\n
#Trip Information\n
\n" 
...
for (i in seq(nrow(sampling))) {
  ...
  if(is.na(current$Time)){
    cat(sprintf(template1, current$Crew.Leader,current$Crew.Members,current$Departure))
  }
  ...
  if(!is.na(current$Time)&is.na(current$class)){
    ...
    rm(dframe)
    cat(sprintf(template2))
    cat(sprintf(template4,current$phone,current$Marine.Radio,current$Distress.Flag,current$Distress.Light,current$Smoke.Dye.Signals,current$Flares,current$Horn,current$Signal.Mirror,current$Fire.Extinguisher.s.,
                current$Comments,current$holder1,current$holderphone1,current$holder2,current$holderphone2,current$hotel))
