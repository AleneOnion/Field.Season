...
template5.1<-"Marine Radio: %s\n "
template5.2<-"Distress Flag: %s\n "
template5.3<-"Distress Light: %s\n "
template5.4<-"Smoke Dye Signals: %s\n "
template5.5<-"Flares: %s\n "
template5.6<-"Horn: %s\n "
template5.7<-"Signal Mirror: %s\n "
template5.8<-"Fire Extinguisher(s): %s\n "
template6<-"\n Comments: %s\n
...
for (i in seq(nrow(sampling))) {
  ...
  if(!is.na(current$Time)&is.na(current$class)){
    ...
    if(!is.na(current$Marine.Radio)){cat(sprintf(template5.1,current$Marine.Radio))}
    if(!is.na(current$Distress.Flag)){cat(sprintf(template5.2,current$Distress.Flag))}
    if(!is.na(current$Distress.Light)){cat(sprintf(template5.3,current$Distress.Light))}
    if(!is.na(current$Smoke.Dye.Signals)){cat(sprintf(template5.4,current$Smoke.Dye.Signals))}
    if(!is.na(current$Flares)){cat(sprintf(template5.5,current$Flares))}
    if(!is.na(current$Horn)){cat(sprintf(template5.6,current$Horn))}
    if(!is.na(current$Signal.Mirror)){cat(sprintf(template5.7,current$Signal.Mirror))}
    if(!is.na(current$`Fire.Extinguisher(s)`)){cat(sprintf(template5.8,current$`Fire.Extinguisher(s)`))}
