...
template4<-"## On Board Communication and Safety Equipment\n
...
for (i in seq(nrow(sampling))) {
  ...
  if(!is.na(current$class)){
    dframe<-current %>% 
      select(Lake,Y_Coordinate,X_Coordinate,access.site_lat,access.site_lon,Water.Time,Return) %>% 
      distinct() %>% 
      rename(lat=Y_Coordinate,
             lon=X_Coordinate,
             launch.y=access.site_lat,
             launch.x=access.site_lon,
             on_water=Water.Time)
